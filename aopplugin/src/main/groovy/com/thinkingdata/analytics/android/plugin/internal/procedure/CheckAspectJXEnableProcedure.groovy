 ...
 */
package com.thinkingdata.analytics.android.plugin.internal.procedure
 ...
 * @since 2018-04-23
 */
class CheckAspectJXEnableProcedure extends AbstractProcedure {
    ...
    boolean doWorkContinuously() {
        ...
        for (String filter : tdExtensionConfig.excludes) {
            if (filter == "*" || filter == "**") {
                isExcludeAll = true
                break
            }
        ...
        for (String filter : tdExtensionConfig.includes) {
            if (filter == "*" || filter == "**") {
                isIncludeAll = true
                break
            }
        ...
        if (isIncludeAll) {
            tdExtensionConfig.includes.clear()
        }

        //aspectjx disabled
        if (!tdExtensionConfig.enabled || isExcludeAll) {
            TDUtils.doWorkWithNoAspectj(transformInvocation)
            return false
        }
