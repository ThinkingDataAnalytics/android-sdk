 ...
 */
package com.thinkingdata.analytics.android.plugin.internal.cache
...
import com.thinkingdata.analytics.android.plugin.internal.model.JarInfo
...
class VariantCache {
    ...
    VariantCache(Project p, TDCache cache, String variantName) {
        this.project = p
        this.variantName = variantName
        this.tdCache = cache
        this.tdCache.put(variantName, this)
    ...
    private void init() {
        ...
        if (includeJarConfig.exists()) {
            List<JarInfo> jarInfoList = TDUtils.optFromJsonString(FileUtils.readFileToString(includeJarConfig), new TypeToken<List<JarInfo>>(){}.getType())
    ...
    void commitIncludeJarConfig() {
        ...
        FileUtils.write(includeJarConfig, TDUtils.optToJsonString(jarInfoList), "UTF-8")
    }
