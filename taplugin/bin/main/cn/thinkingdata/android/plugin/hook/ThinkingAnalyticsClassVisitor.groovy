...
import cn.thinkingdata.android.plugin.hook.push.ThinkingDataPushInjected
...
class ThinkingAnalyticsClassVisitor extends ClassVisitor {
    ...
    void visitEnd() {
        ...
        if (!transformHelper.extension.disableTrackPush && !isFoundOnNewIntent && ThinkingAnalyticsUtil.isActivityClass(mSuperName)) {
            ThinkingDataPushInjected.addOnNewIntent(classVisitor)
        }
    ...
    MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
        if (classNameAnalytics.isSensitiveInfoAPI && transformHelper.enableTASensitiveInfoFilter) {
            if (transformHelper.disableMethods.contains(name)) {
                return null
            }
        }

        if (ThinkingAnalyticsUtil.isActivityClass(mSuperName) && name == "onNewIntent" && desc == "(Landroid/content/Intent;)V") {
        ...
        ThinkingAnalyticsDefaultMethodVisitor thinkingAnalyticsDefaultMethodVisitor = new ThinkingAnalyticsDefaultMethodVisitor(methodVisitor, access, name, desc) {
            ...
            protected void onMethodEnter() {
                 ...
                 * 处理推送hook事件
                 */
                if (!ThinkingAnalyticsUtil.isStatic(access) && !transformHelper.extension.disableTrackPush) {
                    ThinkingDataPushInjected.handlePushEvent(methodVisitor, mSuperName, nameDesc)
                }
